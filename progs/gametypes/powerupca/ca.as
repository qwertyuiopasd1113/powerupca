...
// POWERUP CA:
String POWERUPS_VERSION = "1.4.1.1";
...
class cCARound
{
    ...
    void think()
    {
        ...
        for ( int i = 0; i < maxClients; i++ )
        {
            ...
            if ( ent.client.state() >= CS_SPAWNED && ent.team != TEAM_SPECTATOR )
            {
                ...
                if ( ent.isGhosting() && ent.health > 0.0f  )
                    ent.health = 0.0f;
            }
        }
        ...
        if ( this.roundStateEndTime != 0 )
        {
            ...
            if ( this.countDown > 0 )
            {
                ...
                if ( this.countDown <= 4)
                    G_CenterPrintMsg( null, POWERUPS_getRandomPowerupName() + "\n\n" + String( this.countDown ) );
            }
        }
    ...
    void playerKilled( Entity @target, Entity @attacker, Entity @inflictor )
    {
        ...
        {
            POWERUPS_clearPowerupState(target);
            cPowerUp @attackerPwr = @powerUp[attacker.playerNum];

            if ( attacker.isGhosting() ) {
                G_PrintMsg( target, "You were fragged by the late " + attacker.client.name + "\n" );
            }
			else if ( gametype.isInstagib )
			{
